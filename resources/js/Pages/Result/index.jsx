...
export default function TaskResult({ tasks, users, userName }) {
    ...
    const filteredTasks = tasks
        ...
        .filter(task => 
            ...
            task.penanggung_jawab.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const handleSubmit = (e) => {
        ...
        put(route("it_submit.update", { result: data.uuid }), {
            onSuccess: () => {
            ...
            header={
                <div className="bg-gradient-to-r from-blue-600 to-purple-700 p-6 rounded-2xl text-white shadow-lg mb-8">
                    ...
                    <p className="text-lg opacity-90">
                        Review completed tasks and provide feedback to your team members
                    </p>
                </div>
            }
        >
                ...
                <div className="max-w-[2000px] mx-auto px-4 py-8">
                    ...
                    <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
                        ...
                        <div className="mt-6 flex flex-wrap gap-2">
                            {["In Review", "Approved", "Rejected", "Cancel"].map((status) => (
                                <button
                                    key={status}
                    ...
                    <div className="bg-white shadow-lg rounded-2xl overflow-hidden">
                        ...
                        {filteredTasks.length === 0 ? (
                            <div className="text-center py-12 bg-gradient-to-r from-gray-50 to-blue-50">
                                ...
                                <h3 className="text-lg font-medium text-gray-700 mb-2">No {statusFilter.toLowerCase()} tasks</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
                                {filteredTasks.map((task, index) => (
                                    <div key={task.uuid} className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-300">
                                        {/* Card Header */}
                                        <div className="p-4 border-b border-gray-100">
                                            <div className="flex justify-between items-start">
                                                <h3 className="font-bold text-gray-800 text-lg line-clamp-2">{task.task_title}</h3>
                                        ...
                                        <div className="p-4">
                                            <div className="grid grid-cols-2 gap-4 mb-4">
                                                <div>
                                                    ...
                                                    <p className="font-medium text-gray-800">{task.penanggung_jawab}</p>
                                                </div>
                                                <div>
                                                    <p className="text-xs text-gray-500 uppercase">Company</p>
                                                    <p className="font-medium text-gray-800">{task.company}</p>
                                                </div>
                                                <div>
                                                    <p className="text-xs text-gray-500 uppercase">Format</p>
                                                    <p className="font-medium text-gray-800">{task.task_format}</p>
                                                </div>
                                                <div>
                                                    <p className="text-xs text-gray-500 uppercase">Deadline</p>
                                                    <p className="font-medium text-gray-800">{formatDate(task.deadline)}</p>
                                                    <p className={`text-xs ${getDaysUntilDeadline(task.deadline) < 3 ? 'text-red-500' : 'text-gray-500'}`}>
                                            ...
                                            <div className="bg-blue-50 p-3 rounded-lg mb-4">
                                                <p className="text-xs text-blue-700 uppercase font-medium mb-1">Submission Link</p>
                                                <a
                                        ...
                                        <div className="p-4 bg-gray-50 border-t border-gray-100">
                                            {task.status === "In Review" ? (
                                                ...
                                                <div className="text-center text-sm text-gray-500">
                                                    Reviewed on {formatDate(task.updated_at)}
                                                </div>
                                            )}
                                        </div>
                                    </div>
                            ...
                            <div className="p-6 space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="bg-gray-50 p-4 rounded-lg">
                                        <h4 className="font-semibold text-gray-700 mb-2">Task Details</h4>
                                        <p className="text-sm text-gray-600">{selectedTask.task_title}</p>
                                    </div>
                                    <div className="bg-gray-50 p-4 rounded-lg">
                                        <h4 className="font-semibold text-gray-700 mb-2">Assignee</h4>
                                        <p className="text-sm text-gray-600">{selectedTask.penanggung_jawab}</p>
                                    </div>
                                    <div className="bg-gray-50 p-4 rounded-lg">
                                        <h4 className="font-semibold text-gray-700 mb-2">Company</h4>
                                        <p className="text-sm text-gray-600">{selectedTask.company}</p>
                                    </div>
                                    <div className="bg-gray-50 p-4 rounded-lg">
                                        <h4 className="font-semibold text-gray-700 mb-2">Deadline</h4>
                                        <p className="text-sm text-gray-600">{formatDate(selectedTask.deadline)}</p>
                                    </div>
                                </div>
                                ...
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    <div>
                                        <h4 className="font-semibold text-gray-700 mb-4">Review Decision</h4>
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                            {["Approved", "Rejected", "Cancel"].map((status) => (
                                                <label
                                                    key={status}
                                                    className={`flex flex-col items-center p-4 border-2 rounded-xl cursor-pointer transition-all duration-200 ${
                                                        data.status === status
                                                            ? `${getStatusColor(status)} text-white shadow-md border-transparent`
