                    ...
                    <div className="bg-white overflow-hidden shadow-xl rounded-2xl border border-gray-100">
                        <div className="p-6 md:p-8">
                            ...
                            <div className="mb-8 p-5 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl border border-indigo-100">
                                <h2 className="text-2xl md:text-3xl font-bold text-indigo-800">{event}</h2>
                                <div className="text-lg text-indigo-600 mt-1">{tanggal}</div>
                            ...
                            <div className="mb-8 flex flex-col md:flex-row gap-4 items-start md:items-center">
                                <div className="w-full md:w-auto">
                                    <InputLabel className="text-gray-700 font-medium">Search Equipment:</InputLabel>
                                    <div className="relative mt-1">
                                        ...
                                        <input
                                            ...
                                            className="pl-10 block w-full md:w-80 rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                        />
                                    </div>
                                </div>
                                
                                <div className="w-full md:ml-auto md:w-auto">
                                    <InputLabel className="text-gray-700 font-medium">Filter by Category:</InputLabel>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                                ...
                                <div className="lg:col-span-1">
                                    <div className="sticky top-4 p-4 bg-gray-50 rounded-xl shadow-sm border border-gray-200">
                                        ...
                                        <ul className="space-y-2">
                                            {categories.map(category => (
                                                <li key={category}>
                                                    ...
                                                    >
                                                        <span>{category}</span>
                                                        <span className={`bg-indigo-500 text-white text-xs px-2 py-0.5 rounded-full ${activeCategory === category ? '' : 'opacity-0'}`}>
                                        ...
                                        {itemsByCategory[activeCategory]?.length > 0 ? (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {itemsByCategory[activeCategory].map((item) => (
                                                    <div key={item.id} className={`p-4 bg-white rounded-xl border transition-all ${data.items?.[item.id]?.checked ? 'border-indigo-300 shadow-md' : 'border-gray-200 hover:border-gray-300'}`}>
                                                        <div className="flex items-start">
                                                            ...
                                                            <label htmlFor={item.id} className="ml-3 flex-1">
                                                                <span className={`block font-medium ${data.items?.[item.id]?.checked ? 'text-indigo-700' : 'text-gray-800'}`}>
                                                                    {item.items}
                                                                </span>
