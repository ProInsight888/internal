...
export default function create({ userName, dataEvent, event, tanggal, items }) {
    const { data, setData, post, errors, processing } = useForm({
        ...
        items: items.reduce((acc, e) => {
            acc[e.id] = {
                id: e.id,
                name: e.name,
                checked: dataEvent.some((data) => data.items === e.name),
                category: e.category,
                quantity: dataEvent.find((data) => data.items === e.name)?.quantity || 0
            };
            return acc;
        }, {}),
    });
    ...
    const categoryLabels = {
        ...
        "lensa": "Lensa",
        "audio": "Audio",
        "cable audio": "Cable Audio",
        "gimbal": "Gimbal",
        "drone": "Drone",
        "lighting": "Lighting",
        "battery drone": "Battery Drone",
        "tripod lighting": "Tripod Lighting",
        "tripod": "Tripod",
    ...
    function submit(e) {
        ...
        router.delete(
            route("data_collection.destroy", event),
            {
                preserveScroll: true,
                onSuccess: () => {
                    router.post(route("check.store"), data);
                },
                onError: (errors) => {
   ...
   const handleItemCheck = (itemId, category, checked) => {
        setData("items", {
            ...
            [itemId]: {
                ...
                quantity: checked ? (data.items?.[itemId]?.quantity || 1) : 1
            },
        });
    };
