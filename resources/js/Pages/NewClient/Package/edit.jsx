...
export default function edit({packages, cicilan_package}) {
    ...
    const { data, setData, put, post, processing, errors } = useForm({
        ...
        payment_phase: cicilan_package.map((c, i) => ({
            installment: c.cicilan || "",
            date: c.tanggal || "",
            installment_status: c.status_cicilan || "false",
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route("package.update", packages.uuid), {
                                            ...
                                            (phase, index) => (
                                                <div key={index}>
                                                    <div className="flex gap-5">
                                                        <input type="checkbox" name={`phase-${index}`} id={`phase-${index}`} 
                                                        checked={ 
                                                            phase.installment_status === "true" ? true : false
                                                        }
                                                        onChange={(e) =>{
                                                            const update_check = [...data.payment_phase];
                                                            update_check[index] = {
                                                                ...update_check[index],
                                                                installment_status: e.target.checked ? "true" : "false"
                                                            };
                                                            setData('payment_phase', update_check);
                                                        }}
