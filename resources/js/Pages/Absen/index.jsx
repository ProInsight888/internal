...
const AttendanceTable = ({ data, users }) => {
    // Add users prop
    ...
    // Function to get current user name from user_id
    const getCurrentUserName = (user_id) => {
        console.log(user_id)
                    ...
                    {data.map((absen, index) => (
                        ...
                        >
                            ...
                            <td className="px-2 md:px-4 py-2 md:py-3 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                                {getCurrentUserName(absen.user_id)}{" "}
                            </td>
                            <td className="px-2 md:px-4 py-2 md:py-3">
...
// Main Component
// Main Component
export default function AbsenceDashboard({ absens, users }) {
    ...
    // State management - now storing user ID instead of name
    const [sortMonth, setSortMonth] = useState(currentMonth);
    const [sortUserId, setSortUserId] = useState(""); // Changed from sortName to sortUserId
    const { data, setData, post, errors, processing } = useForm({
        from_date: "",
        end_date: "",
    });

    // Filter and sort data - now using user_id for filtering
    const filteredData = absens
        .filter(
            (absen) =>
                absen.tanggal.split("-")[1] === sortMonth &&
                (sortUserId === "" || absen.user_id?.toString() === sortUserId) // Compare with user_id
        )
    ...
    function submit(e) {
        ...
        const query = new URLSearchParams({
            ...
            users: userArray,
            sortUserId: sortUserId, // Updated to sortUserId
        }).toString();
                        ...
                        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-4 md:p-6 border-b border-blue-200 dark:border-blue-700">
                            <div className="flex flex-col gap-4 mb-4 md:mb-6">
                                ...
                                <div className="w-full">
                                    <NameFilter
                                        users={users} // Pass the full users array with IDs and names
                                        value={sortUserId} // Pass user ID instead of name
                                        onChange={
                                            (e) => setSortUserId(e.target.value) // Set user ID instead of name
                                        }
                                    />
                                </div>
                            </div>
